...
import rs.ac.bg.etf.aor2.replacementpolicy.*;
...
//        int[] blockSizes = {1, 2, 4, 8, 16, 32, 64};
        ...
        // ZA VALGRIND TEST (jer su male matrice)
        // NOTE: Pseudo LRU u simulatoru samo podrzava SA=4, pa ovo treba promeniti ako se zeli i on testirati
        int[] setAsocs = {8};
        int[] blockNums = {128};
        int[] blockSizes = {8};

        // ZA TEST_MAT_MUL (ako se koristi default - ovde su vece matrice)
//        int[] setAsocs = {8};
//        int[] blockNums = {512};
//        int[] blockSizes = {64};

        // algoritmi zamene koji se testiraju
        IReplacementPolicy[] policies = new IReplacementPolicy[]{
                new FIFOReplacementPolicy(),
                new LFUReplacementPolicy(),
                new LRUKReplacementPolicy(),
                new LRUReplacementPolicy(),
                new MPLRUReplacementPolicy(),
                new NRUReplacementPolicy(),
                new OneBitReplacementPolicy(),
                new SIDEReplacementPolicy()
        };

        // izlazni fajl
        fNameStatistic = "Statistika - Valgrind Test";

        try (FileWriter fileWriter = new FileWriter(fNameStatistic)) {
            fileWriter.append("Statistika ").append(String.valueOf(Calendar.getInstance().getTime())).append(":\n\n");
        ...
        for (int sa = 0; sa < setAsocs.length; sa++) {
            ...
            for (int bn = sa; bn < blockNums.length; bn++) {
                ...
                for (int bs = 0; bs < blockSizes.length; bs++) {
                    ...
                    for (IReplacementPolicy policy : policies)
                        createCacheAndTest(adrSize, setAsoc, blockNum, blockSize, fNameStatistic, policy);
    ...
    private static void createCacheAndTest(int adrSize, int setAsoc, int blockNum, int blockSize, String fNameStatistic, IReplacementPolicy policy) {
        ...
        // ako se zeli posmatrati zasebno hit/miss rate instrukcijskoj kesa ovo je potrebno
...
//        myNativeTest(cache);
        myValgrindTest(cacheL2);
//        testMatrixMultiplicationNoBlocking(cacheL2);
//        testMatrixMultiplicationBlocking(cacheL2);


        long hits_cnt = cacheL2.getCacheHitNum();
        long miss_cnt = cacheL2.getCacheMissNum();
...
//        printStatisticsToFile(cacheL2, fNameStatistic);

        printStatisticsWithPolicyToFile(cacheL2, fNameStatistic);
    ...
    private static void printStatisticsWithPolicyToFile(ICacheMemory cache, String fName) {
        ...
        try (FileWriter fileWriter = new FileWriter(fName, true)) {
            fileWriter.append(String.format(
                    "%s:\n" +
                            ...
                            "Hit rate:\t %.2f\n" +
                            "----------------------------------------\n",
                    cache.getReplacementPolicy().getName(),
                    cache.getBlockSize(),
                    cache.getBlockNum(),
                    cache.getSetAsociativity(),
                    cache.getCacheHitNum(),
...
//        testTrace(cache, loader);
    ...
    public static void testTrace(ICacheMemory cache, ITraceLoader loader) {
        ...
        while (loader.hasOperationToLoad()) {
            ...
            if (!loader.isInstructionOperation()) { // preskoci operacije instrukcija - kvari statistiku
